...
; (function($) {
    ...
    $.fn.bcalendar = function(option) {
        ...
        function ResizeView() {
            ...
            if (_viewType == "day" || _viewType == "week") {
                ...
                $dvtec.parent().height(calH + dvwkH);
            }
            else if (_viewType == "month") {
            	$calmonthcc = $("#cal-month-cc");
            	$mvcontainer = $("#mvcontainer");
            	$mvEventContainer = $("#mvEventContainer");
                var calH = option.height - 26;
                
                $mvEventContainer.height(calH + 3);
                
                $mvEventContainer.parent().height(calH + $calmonthcc.height() + $mvcontainer.height());
        ...
        var c = {
            ...
            param:function(viewtype){
            	var showday = option.showday;
            	var zone = new Date().getTimezoneOffset() / 60 * -1;
            	var config = { view: viewtype, weekstartday: option.weekstartday, theme: option.theme };
                var param = new Object();
                param.showdate = dateFormat.call(showday, i18n.xgcalendar.dateformat.fulldayvalue);
                param.timezone = zone;
                param.viewtype = viewtype;
                var days = [];
                if (viewtype === "day") {
                    var show = dateFormat.call(showday, i18n.xgcalendar.dateformat.Md);
                    days.push({ display: show, date: showday, day: showday.getDate(), year: showday.getFullYear(), month: showday.getMonth() + 1 });
                    param.datestrshow = CalDateShow(days[0].date);
                }
                else if (viewtype === "week"){
                    var w = 0;
                    var l =7;
                    w = config.weekstartday - showday.getDay();
                    if (w > 0) w = w - 7;
                    
                    var ndate;
                    for (var i = w, j = 0; j < l; i = i + 1, j++) {
                        ndate = DateAdd("d", i, showday);
                        var show = dateFormat.call(ndate, i18n.xgcalendar.dateformat.Md);
                        days.push({ display: show, date: ndate, day: ndate.getDate(), year: ndate.getFullYear(), month: ndate.getMonth() + 1 });
                    }
                    param.datestrshow = CalDateShow(days[0].date, days[l - 1].date);
                }else if (viewtype === "month"){//month
                	 var firstdate = new Date(showday.getFullYear(), showday.getMonth(), 1);
                     var diffday = config.weekstartday - firstdate.getDay();
                     var showmonth = showday.getMonth();
                     if (diffday > 0) {
                         diffday -= 7;
                     }
                     ...
                     option.datestrshow = CalDateShow(startdate, enddate);
                    param.datestrshow = CalDateShow(startdate, enddate);
                }
                return param;
            }
        };
    ...
    /**wangfan
     * @description used for customized list view
     */
    $.fn.getParam = function(viewtype) {
    	 if (this[0].bcal) {
             return this[0].bcal.param(viewtype);
         }
         return null;
    };
    
    $.fn.resize = function(height) {
        return this.each(function() {
            if (this.bcal) {
                this.bcal.rz(height);
            }
