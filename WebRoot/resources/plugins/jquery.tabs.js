...
//			header.addClass('tabs-header-left tabs-header-right');
	...
	function createTab(container, pp, options) {
		...
		//wangfan add support for tab id
		opts.tab = $('<li></li>').attr("tab-id",options.id).appendTo(tabs);	// set the tab object in panel options
		opts.tab.append(
...
//		setProperties(container);
	...
	/* wangfan add custom method here*/
	function silentSelectTab(container, which){
		var opts = $.data(container, 'tabs').options;
		var tabs = $.data(container, 'tabs').tabs;
		var selectHis = $.data(container, 'tabs').selectHis;
	...
	$.fn.tabs.methods = {
		...
		addSub: function(jq, options){
			return jq.each(function(){
				addTab($(this).append("<div class='subtabdiv'></div>"), options);
			});
		},
		...
		/*wangfan add custom method here*/
		silentSelect: function(jq, which){
			return jq.each(function(){
				silentSelectTab(this, which);
			});
		},
