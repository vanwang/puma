...
(function(){
	var modules = {
		...
		/** plugins for daylog calendar*/
		daylogcalendarcore:{
			js:'calendar/Common.js'
		},
		daylogcalendardatepickerlang:{
			js:'calendar/datepicker_lang_HK.js'
		},
		daylogcalendardatepicker:{
			js:'calendar/jquery.datepicker.js',
			css:'calendar/dp.css'
		},
		daylogcalendaralert:{
			js:'calendar/jquery.alert.js',
			css:'calendar/alert.css'
		},
		daylogcalendardialog:{
			js:'calendar/jquery.ifrmdailog.js',
			css:'calendar/calendar.css'
		},
		daylogcalendarlang:{
			js:'calendar/wdCalendar_lang_HK.js'
		},
		daylogcalendar:{
			js:'calendar/jquery.calendar.js',
			css:'calendar/main.css',
			dependencies:['daylogcalendarcore','daylogcalendardatepickerlang','daylogcalendardatepicker','daylogcalendaralert','daylogcalendardialog','daylogcalendarlang']
		},
		daylogslidernav:{
			js:'slidernav/slidernav.js',
			css:'slidernav/slidernav.css'
		},
		/** plugins added by wangfan for puma cms*/
		jquerycore:{
		...
		jquerymouse:{
			...
			dependencies:['jquerywidget']
		},
		jquerysortable:{
			js:'jquery.ui.sortable.js',
			dependencies:['jquerycore','jquerywidget','jquerymouse']
		},
		PumaApps:{
			js:'puma.app.js'
		},
		PumaInit:{
			js:'puma.init.js',
			css:'puma.css',
			dependencies:['PumaApps','tabs','tree']
		},
		SystemHomeModule:{
			js:'puma.systemhome.js'
		},
		UserManagementModule:{
			js:'puma.usermanagement.js',
			dependencies:['datagrid','dialog','form','messager','combotree']
		},
		UserLogManagementModule:{
			js:'puma.userlogmanagement.js',
			dependencies:['datagrid','dialog','form','messager','combotree']
		},
		ChangeSkinModule:{
			js:'puma.changeskin.js'
		},
		AditionModule:{
			js:'puma.adition.js'
		},
		ResourceManagementModule:{
			js:'puma.resourcemanagement.js',
			dependencies:['treegrid','dialog','combobox','messager','form']
		},
		RoleManagementModule:{
			js:'puma.rolemanagement.js',
			dependencies:['layout','dialog','tree','messager','form','jquerysortable']
		},
		MenuManagementModule:{
			js:'puma.menumanagement.js',
			dependencies:['layout','dialog','tree','messager','form','splitbutton','jquerysortable']
		},
		DaylogModule:{
			js:'puma.daylog.js',
			dependencies:['daylogcalendar','daylogslidernav']
		},
		QrCodeModule:{
			js:'puma.qrcode.js'
		}
	...
	if (window.jQuery){
		jQuery(function(){
			$.ajax( {
                "dataType": 'JSON',
                "type": "POST",
                "url": "member/memberpreference.json",
                "contentType": "application/x-www-form-urlencoded; charset=UTF-8",
                "success": function(data){
            		 easyloader.preferences = data.message;
            		 easyloader.theme = data.message.theme;
            		 easyloader.load(['parser','PumaInit'], function(){
         				jQuery.parser.parse();
         			});
                },
                "error":function(data){
                	$.unloading();
                	$.messager.alert('错误','加载用户偏好设置失败,请刷新重试，错误原因:<br>'+data.statusText,'error');  
    			}
              } );
