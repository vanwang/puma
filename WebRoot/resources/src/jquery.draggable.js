...
//		}
	...
	function doDown(e){
		//wangfan,change this to another place
		//isDragging = true;
		var state = $.data(e.data.target, 'draggable');
		var opts = state.options;
		...
		//wangfan we should not fire startDrag event in mouse down phase
	...
	function doMove(e){
		//wangfan
		var state = $.data(e.data.target, 'draggable');
		//wangfan move the isDragging from doDown method to doMove
		if(!isDragging){
			isDragging = true;
			state.options.onStartDrag.call(e.data.target, e);
		}
		drag(e);
